...
export class BaseHands {
  ...
  clickOnHrefElement(id: string) {
    cy.contains(id).click()
    return this;
  }
  clickDomElementUsingContains(id: string) {
    cy.contains(id).click({force:true});
    return this;
  }
  checkBoxUsingName(type: string,name: string) {
    cy.get(`[type='checkbox']`).check(name);
    return this;
  }
  ...
  clickOnDomElementInSameTab(url: string) {
      cy.visit(url);
